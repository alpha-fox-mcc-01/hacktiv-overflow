      ...
      <div class="font-sans flex items-center justify-center bg-blue-darker w-full py-8">
        <div class="overflow-hidden bg-white rounded max-w-xs w-full shadow-lg  leading-normal">
            <a class="block group hover:bg-blue p-4 border-b">
                ...
                <p v-for="question in myQuestions" :key="question._id" class="text-grey-darker mb-2 group-hover:text-white">- {{question.title}} <a><i  @click="deleteQuestion(question._id)" class="fa fa-trash" aria-hidden="true"></i></a></p> 
            </a>
             <a class="block group hover:bg-blue p-4 border-b">
                <p class="font-bold text-lg mb-1 text-black group-hover:text-white">My Answers</p>
                <p v-for="answer in myAnswers" :key="answer._id" class="text-grey-darker mb-2 group-hover:text-white">- {{answer.title}} <a><i @click="editAnswer(answer._id, answer.title, answer.content)" class="fa fa-pencil" aria-hidden="true"></i></a> | <a><i @click="deleteAnswer(answer._id)" class="fa fa-trash" aria-hidden="true"></i></a></p> 
            </a>
        </div>
      </div>
    </div>        
...
export default {
    ...
    methods: {
      ...
      editAnswer(id, title, content) {
        let payload = {
          title: title,
          content: content
        }
        this.$store.dispatch('preEditAnswer', payload)
